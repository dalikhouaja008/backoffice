...
class MenuItem {
  ...
  final String? path;
  final bool? blank;
  final List<String> allowedRoles; // Paramètre requis
  ...
  MenuItem({
    ...
    this.path,
    this.blank,
    required this.allowedRoles, // Utilisation du paramètre allowedRoles comme requis
    this.childList,
  });
}
...
class SidebarService {
  ...
  static Future<List<MenuGroup>> loadMenu([String? assetPath]) async {
    try {
      ...
      final String menuJson = await rootBundle.loadString(
        assetPath ?? 'assets/routes/menu_route_en.json'
      );
  ...
  static Future<List<MenuGroup>> getMenuForRole(String userRole) async {
    try {
      // Charger le fichier JSON du menu
      final String menuJson = await rootBundle.loadString('assets/routes/menu_route_en.json');
      final List<dynamic> menuData = json.decode(menuJson);
  ...
  static List<dynamic> _filterMenuByRole(List<dynamic> menuData, String userRole) {
      ...
      if (_hasAccess(group['roles'], userRole)) {
        ...
        for (var menuItem in group['menuList']) {
          if (_hasAccess(menuItem['roles'], userRole)) {
            if (menuItem.containsKey('childList') && menuItem['childList'] != null) {
  ...
  static bool _hasAccess(List<dynamic>? roles, String userRole) {
    ...
    return roles.contains(userRole) || roles.contains('all') || userRole == 'admin';
  }
  ...
  static bool canAccessRoute(String role, String route, List<MenuGroup> menuGroups) {
